...
class Vehicle {
  ...
  Vehicle( PVector l, float ms, float mf, PVector accel) {
    location = l.get();
    r = 4.0;
    maxspeed = ms;
    maxforce = mf;
  ...
  void follow(Path p) {
    ...
    if (location.dist(p.points.get(p.points.size()-1)) < maxStopDistance) {
      stopped = true;
    }
  }
